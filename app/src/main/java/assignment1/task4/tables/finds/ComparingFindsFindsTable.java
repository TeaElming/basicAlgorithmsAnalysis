...
public class ComparingFindsFindsTable {
  ...
  public static void main(String[] args) {
    ...
    createAndShowTables(size, numberOfFinds);
  }
  ...
  public static void createAndShowTables(int size, int[] numberOfFinds) {
    String[] columnNames = { "Number of Finds", "Avg Time (ns)", "Error Margin (%)" };
    String[] comparativeColumnNames = { "Number of Finds", "Avg QF (ns)", "Avg QU (ns)", "Avg WU (ns)" };
    ...
    for (int finds : numberOfFinds) {
      ...
      MeasurementResult qfResult = measureMultipleRuns(() -> measureFindTimeQF(size, finds));
      qfModel.addRow(new Object[] { finds, qfResult.avgTime, qfResult.errorMargin, 0 });

      // Measure times for QuickUnion
      MeasurementResult quResult = measureMultipleRuns(() -> measureFindTimeQU(size, finds));
      quModel.addRow(new Object[] { finds, quResult.avgTime, quResult.errorMargin, 0 });

      // Measure times for WeightedUnion
      MeasurementResult wuResult = measureMultipleRuns(() -> measureFindTimeWU(size, finds));
      wuModel.addRow(new Object[] { finds, wuResult.avgTime, wuResult.errorMargin, 0 });

      // Add to comparative table
      comparativeModel.addRow(new Object[] { finds, qfResult.avgTime, quResult.avgTime, wuResult.avgTime });
    }

    // Create the JFrame to hold all tables
    JFrame frame = new JFrame("Comparing Find Algorithms with Varying Number of Finds");
