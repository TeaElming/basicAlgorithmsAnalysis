...
public class ComparingUnionsUnionsTable {

  public static void main(String[] args) {
    // Test with consistent size and varying unions
    int size = 10000;

    int startNumber = 0; // Starting number of unions
    int endNumber = 9999; // Ending number of unions
    ...
    createAndShowTables(size, numberOfUnions);
  }

  // Method to generate an array of numbers in increments
  public static int[] generateNumberOfUnions(int start, int end, int increment) {
    ...
    for (int i = 0; i < length; i++) {
      unions[i] = start + (i * increment); // Populate the array with increments
    }
    return unions;
  }

  // Create and display tables for each algorithm and a comparative table
  public static void createAndShowTables(int size, int[] numberOfUnions) {
    String[] columnNames = { "Number of Unions", "Avg Time (ns)", "Error Margin (%)" };
    String[] comparativeColumnNames = { "Number of Unions", "Avg QF (ns)", "Avg QU (ns)", "Avg WU (ns)" };
    ...
    for (int unions : numberOfUnions) {
      ...
      MeasurementResult qfResult = measureMultipleRuns(() -> measureUnionTimeQF(size, unions));
      qfModel.addRow(new Object[] { unions, qfResult.avgTime, qfResult.errorMargin, 0 });

      // Measure times for QuickUnion
      MeasurementResult quResult = measureMultipleRuns(() -> measureUnionTimeQU(size, unions));
      quModel.addRow(new Object[] { unions, quResult.avgTime, quResult.errorMargin, 0 });

      // Measure times for WeightedUnion
      MeasurementResult wuResult = measureMultipleRuns(() -> measureUnionTimeWU(size, unions));
      wuModel.addRow(new Object[] { unions, wuResult.avgTime, wuResult.errorMargin, 0 });

      // Add to comparative table
      comparativeModel.addRow(new Object[] { unions, qfResult.avgTime, quResult.avgTime, wuResult.avgTime });
    }

    // Create the JFrame to hold all tables
    JFrame frame = new JFrame("Comparing Union Algorithms with Varying Number of Unions");
